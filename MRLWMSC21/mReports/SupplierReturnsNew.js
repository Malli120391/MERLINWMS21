...
myApp.controller('SupplierReturnsNew', function ($scope, $http) {
   ...
   // }
    $scope.Getgedetails = function () {
        ...
        var httpreq = {
            method: 'POST',
            url: 'SupplierReturnsNew.aspx/GetBillingReportList',
            headers: {
                'Content-Type': 'application/json; charset=utf-8',
                'dataType': 'json'
            },
            data: {
                'FromDate': fromdate, 'ToDate': textfieldname, 'TenantId': Tenantid, 'WareHouse': Warehouseid
            },
            async: false
        }
   ...
   // $scope.Getgedetails();
    ...
    $scope.export = function () {
        ...
        $("#tbldata").empty();
        $("#tbldata").append("<table><thead><tr><td><img src='../Images/INV_TraceabilityAssured.png' style='height: 56px;' border='0'></td></tr><tr><th class='aligntext'>Supplier</th><th class='aligntext'>Tenant</th><th class='aligntext'>PartNumber</th><th class='aligntext'>InvoiceNo</th><th class='aligntext'>ReturnDate</th><th class='aligntext'>ReturnQty</th><th class='aligntext'>UoM</th></thead><tbody>");
        if (table != null && table.length > 0) {      
        for (var i = 0; i < table.length; i++) {
            $("#tbldata").append("<tr></td><td class='aligntext'>" + table[i].Supplier + "</td><td class='aligntext'>" + table[i].Tenant + "</td><td class='aligntext'>" + table[i].PartNumber + "</td><td class='aligntext'>" + table[i].InvoiceNo + "</td><td class='aligntext'>" + table[i].ReturnDate + "</td><td class='aligntext'>" + table[i].ReturnQty + "</td><td class='aligntext'>" + table[i].UoM+ "</td></tr>");
        }

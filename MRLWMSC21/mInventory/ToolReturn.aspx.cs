...
namespace MRLWMSC21.mInventory
{
    ...
    //Use case ID:  Tool Returns_UC_019


    public partial class ToolReturn : System.Web.UI.Page
    {
        ...
        {
            if (!cp.IsInAnyRoles(CommonLogic.GetRolesAllowed(CommonLogic.GetRolesAllowedForthisPage("Tool Returns"))))
            {
                //test case ID (TC_008)
                //Only authorized user can access tool returns page

                Response.Redirect("Login.aspx?eid=6");
            }
            if (!IsPostBack)
            {
                DesignLogic.SetInnerPageSubHeading(this.Page, "Tool Returns");
            }
        }
        ...
        private void AddDynamicColumns()
        {
            ...
            try
            {
                ...
                while (rsMSPList.Read())
                {
                    ...
                    gvToolReturn.Columns.Add(field);
                }
        ...
        {
            //test case ID (TC_012)
            //Get details based on employee and issue Number

            if (atcEmployee.Text.Trim().Length > 0 && hifEmployee.Value.Trim().Length > 0 && atcSONumber.Text.Trim().Length > 0 && hifSONumber.Value.Trim().Length > 0 && atcStoreNo.Text.Trim().Length > 0 && hifStoreNo.Value.Trim().Length > 0)
            {
                Build_ToolReturn(Build_ToolReturn());
            }
            else
            {
                resetError("Employee, Issue No. and Store cannot be empty", true, false);
            }
        }
        ...
        protected void lnkReturn_Click(object sender, EventArgs e)
        {
            ...
            //test case ID (TC_019)
            //return inbound created based on selected line items
            ...
            {
                resetError("Error while Return items",true,false);
                CommonLogic.createErrorNode(cp.UserID + " / " + cp.FirstName, this.Page.ToString(), ex.Source, ex.Message, ex.StackTrace);
                return;
            }


            foreach (GridViewRow row in gvToolReturn.Rows)
            {
            ...
            try
            {
                ...
                cmdcustomerReturn.Append("DECLARE @NewResult int EXEC [dbo].[sp_INV_DummyInboundForToolsReturns] ");
                cmdcustomerReturn.Append("@SOHeaderID=" + hifSONumber.Value);
                ...
                cmdcustomerReturn.Append(",@TenantID=" + cp.TenantID);
            ...
            {
                resetError("Error while updateing", true, false);
                CommonLogic.createErrorNode(cp.UserID + " / " + cp.FirstName, this.Page.ToString(), ex.Source, ex.Message, ex.StackTrace);
            }
        }

        private DataSet Build_ToolReturn()
        {
            DataSet dsToolReturn = null;
            String cMdToolReturn = "sp_INV_GetAvailableItemsForEmployee	 @UserID=" + hifEmployee.Value + ",@SOHeaderID=" + hifSONumber.Value+",@WarehouseID=" + hifStoreNo.Value;
            try
            {
                dsToolReturn = DB.GetDS(cMdToolReturn, false);
            }
            catch (Exception ex)
            {
                resetError("Error while loading",true,false);
        ...
        private void Build_ToolReturn(DataSet dsToolReturn)
        {
            gvToolReturn.DataSource = dsToolReturn;
            gvToolReturn.DataBind();
            dsToolReturn.Dispose();
        }
        ...
        private String GetDummyPONumber()
        {
            ...
            try
            {
                NewPONumber = DB.GetSqlS("EXEC [sp_SYS_GetSystemConfigValue]   @SysConfigKey='dummypoheaderfortoolsreturn_prefix' ,@TenantID=" + cp.TenantID); //"D"+DB.GetSqlS("select SysConfigValue as S from SYS_SystemConfiguration where TenantID=" +cp.TenantID + " and SysConfigKeyID= (select SysConfigKeyID from SYS_SysConfigKey where SysConfigKey=N'purchaseorder.aspx.cs.PO_Prefix') ");
                ...
                //String OldPONumber = DB.GetSqlS("select TOP 1 PONumber as S from ORD_POHeader where POTypeID=8 and TenantID=" + cp.TenantID + "  ORDER BY PONumber desc ");
        ...
        protected void gvToolReturn_RowDataBound(object sender, GridViewRowEventArgs e)
        {
